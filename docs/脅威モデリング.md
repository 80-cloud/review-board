...
> 本書は要件定義書 §0 / §4-1（セキュリティ・認可）の必須要件
...
- 本書が扱わない範囲（現フェーズの対象範囲・将来の本格運用時に再検討、要件 §2-2）：WAF / CloudFront、レート制限、SAST、監査ログの改ざん防止（WORM/署名）、SIEM 連携。
...
## 7. 残存リスク・将来の本格運用時の再検討

現フェーズの対象範囲のため現フェーズで簡素化している項目（要件 §2-2 の横断原則に従い、将来の本格運用時に再検討）：
...
| スクショの実ファイル取扱い | **#123 で実装**：認証必須アップロード・**magic byte 判定**（拡張子/Content-Type 不信）・5MB 上限・**private 隔離＋短命署名 URL**（直アクセス 403） | ウイルススキャン（ClamAV）・CDN 配信・画像変換は将来フェーズ |
...
- [要件定義書.md](./要件定義書.md) §0・§3-2・§4-1（セキュリティ・認可・RBAC・重視した基準）
