...
> **基本スタックは slack-board / sns-board と揃える**（学習資産の踏襲）。review-board 固有の差分は
> ① 言語ランタイムを **Java 25 (LTS)** へ先行採用（要件定義書 §1-6「やったことがないことをするのが
> エンジニアの仕事」）、② セキュリティ・認可の強化（RBAC / IDOR / 依存脆弱性スキャン）である。
...
│                                             │
│   ・REST API (/api/*)                        │
│   ・Spring Security による RBAC + 所有者/cohort 検証 (認可の中核)│
│   ・SPA 配信 (/)                             │
...
詳細は [機能一覧.md](./機能一覧.md)（後日作成）参照。本アプリは認可が強みのため、
...
**バージョン:** **3.5.5** — family（3.5.0）からの **パッチ上げ（踏襲重視）**。
...
- Spring Web（REST API）
- **Spring Security**（セキュリティ・認可の中核）：RBAC（受講生 / 講師）・認可フィルタ・メソッドレベル認可（`@PreAuthorize`）
...
- **監査ログ**：`audit_logs` に「誰が・いつ・誰の資源に・何をしたか」を記録（重視した基準）

### 2-5. 依存脆弱性スキャン（とくに力を入れた領域・CI 常設）
...
無料枠の範囲内で運用（要件定義書 §2-2、月 $0.50 以下）。CloudFront / WAF は現フェーズ（学習）では対象外（将来の本格運用へ移行する場合は再検討。要件定義書 §2-2 横断原則）。
...
| **認可 (RBAC/IDOR)** | Spring Security `@PreAuthorize` + 所有者/cohort 検証 | Spring Boot 内蔵 | ➕ 追加（セキュリティ・認可の中核） |
| DB マイグレーション | Flyway | Spring Boot 内蔵 | ✅ 同一 |
| 依存脆弱性スキャン | OWASP dependency-check | Gradle plugin | ➕ 追加（とくに力を入れた領域・CI 常設） |
...
> **再検討余地（要件定義書 §2-2 横断原則）：** 下表の「現フェーズ判断」と記した不採用は恒久排除でなく、**将来の本格運用へ移行する場合は再検討**する。

| 技術 | 不採用の理由 | 区分 |
|---|---|---|
| Spring Boot 4.0.x | Java 25 ファーストクラス対応で魅力的だが、family（3.5 系）から乖離が大きく学習資産・参考資料の流用が減る。踏襲重視で 3.5.5 を選択（差分最小） | 設計判断 |
...
| CloudFront / WAF | インフラ簡素化（要件定義書 §2-2・§5）。**現フェーズ判断**＝将来の本格運用へ移行する場合は再検討（CDN・WAF・DDoS 対策の必要性を再評価） | 現フェーズ判断 |
...
- すべての依存は §2-5 の脆弱性スキャン対象。High/Critical は 0 件を維持（とくに力を入れた領域）
