...
> エンジニアの仕事」）、② 重点領域（セキュリティ・認可）の強化（RBAC / IDOR / 依存脆弱性スキャン）である。
...
│                                             │
│   ・REST API (/api/*)                        │
│   ・Spring Security による RBAC + 所有者/cohort 検証 (★重点領域)│
│   ・SPA 配信 (/)                             │
...
詳細は [機能一覧.md](./機能一覧.md)（後日作成）参照。本アプリは 重点領域（認可）が強みのため、
...
- Spring Web（REST API）
- **Spring Security**（★重点領域の中核）：RBAC（受講生 / 講師）・認可フィルタ・メソッドレベル認可（`@PreAuthorize`）
...
| **認可 (RBAC/IDOR)** | Spring Security `@PreAuthorize` + 所有者/cohort 検証 | Spring Boot 内蔵 | ➕ 追加（★重点領域の中核） |
| DB マイグレーション | Flyway | Spring Boot 内蔵 | ✅ 同一 |
| 依存脆弱性スキャン | OWASP dependency-check | Gradle plugin | ➕ 追加（★重点領域・CI 常設） |
...
- すべての依存は §2-5 の脆弱性スキャン対象。High/Critical は 0 件を維持（★重点領域）
