...
function ProfileEditModal({ profile, onSaved, onCancel }) {
  ...
  const save = async () => {
    ...
    setErr('');
    try {
      const updated = await updateMyProfile({ bio: bio || null, avatarKey, portfolioUrl: portfolioUrl || null, githubUrl: githubUrl || null });
      onSaved(updated);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/10">
        ...
        <div className="space-y-5 px-6 py-6">
          ...
          {/* GitHub URL（任意・F-PROF 拡張） */}
          <div>
            <label htmlFor="profile-github" className="mac-label">GitHub URL</label>
            <input
              id="profile-github"
              type="url"
              inputMode="url"
              ...
              className="mac-input"
            />
            <p className="mt-1 text-xs text-gray-400">GitHub アカウントの URL を登録できます。</p>
          </div>
        </div>
